...
//		 str="INSERT INTO B (USER_ID,PARTITION_ID )SELECT * FROM ( SELECT -1+2+3, a+1*2+3-4 AS userid,PARTITION_ID AS  PARTITION_ID FROM C WHERE USER_ID>0 WHERE ROWNUM>10 GROUP BY USER_ID )A ORDER BY userid;";
		 str="insert into B select a from (select a,b,c from tableB)source1 where b>0;";
