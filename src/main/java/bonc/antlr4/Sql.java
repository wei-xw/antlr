...
//		 str="INSERT INTO B (USER_ID,PARTITION_ID ) SELECT * FROM (SELECT USER_ID AS userid,PARTITION_ID AS  PARTITION_ID FROM C WHERE USER_ID>0)A WHERE USER_ID>1;";
		 str="INSERT INTO HJ.WXW_ANTLR_DIRECT_TEST (USER_ID,PARTITION_ID,LIST_CODE,CREDIT_ORG,CREDIT_TYPE)SELECT * FROM ( SELECT concat(USER_ID,PARTITION_ID),1+2,count(1), LENGTH(MPFEE)+1*2+3-4 AS a,PARTITION_ID AS  PARTITION_ID FROM HJ.WXW_ANTLR_TEST WHERE USER_ID>0 GROUP BY USER_ID )A where a>0 ;";//ORDER BY userid WHERE ROWNUM>10
